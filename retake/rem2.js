...
//ID:65130500086       Name:Hathaitat Yamchomchuen
...
class Order {
  ...
  calculateTotal() {
    const total = this.items.reduce(
      (pre, cur) => ((pre.price = pre.price + cur.price), 0)
    );
    return total;
  ...
  findItemByName(itemName) {
    const matchingName = this.items.find(
      (findName) => findName.name.toLowerCase() === itemName.toLowerCase()
    );
    if (matchingName) {
      return matchingName;
    } else {
  ...
  isItem(Item) {
    ...
    const CheckPriceAndQuantity = this.items.findIndex(
      (item) =>
